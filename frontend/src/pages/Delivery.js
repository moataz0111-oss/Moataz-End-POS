...
// مكون خريطة السائقين مع أيقونة دراجة نارية
const DriverMap = ({ drivers, mapContainerRef }) => {
  ...
  const updateMarkers = () => {
    ...
    // أيقونة السائق المشغول (لديه طلب)
    const busyMotorcycleIcon = L.divIcon({
      html: `
        <div style="
          width: 50px; 
          height: 50px; 
    ...
    // تحريك الخريطة لتشمل جميع السائقين
  ...
  return null; // المكون لا يرسم شيء مباشرة، يستخدم mapContainerRef
};
